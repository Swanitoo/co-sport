    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      {products.map((product, index) => {
        ...
        const countryFlag = product.user.country
          ? COUNTRIES.find((c) => c.code === product.user.country)?.flag || null
          : null;
          ...
          >
            ...
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <span className="flex max-w-[50%] items-center gap-1 truncate">
                <span className="shrink-0">{getSportIcon(product.sport)}</span>{" "}
                {product.sport}
              </span>
              <span className="flex max-w-[50%] items-center gap-1 truncate">
                <span className="shrink-0">{getLevelIcon(product.level)}</span>{" "}
                {product.level}
              </span>
            </div>
          </ProductCardLink>
