...
export function ChatComponent({
    ...
    async (pageNum: number, silent = false) => {
      ...
      try {
        const response = await fetch(
          `/api/annonces/${productId}/messages?page=${pageNum}&limit=${messageLimit}`
        );
  ...
  useEffect(() => {
    const loadOnlineUsers = async () => {
      try {
        const response = await fetch(`/api/annonces/${productId}/members`);
    ...
    try {
      ...
      try {
        ...
        if (!result.data?.success) {
          throw new Error(
            result.data?.serverError || "Erreur lors de l'envoi du message"
          );
        }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!result.data?.success) {
        ...
        toast.error(
          result.data?.serverError || "Erreur lors de la suppression"
        );
      } else {
